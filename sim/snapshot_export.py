...
def build_snapshot_document(world: World, seed: int, tick: int) -> dict:
    ...
    for cell_id, cell in sorted(world.cells.items(), key=lambda item: int(item[0])):
        ...
        repartition_bourg = repartition_bourg_de_cellule_consultation(
            cid, repartitions_bourg
        )
        if repartition_bourg is None:
            raise SnapshotExportError(f"bourg absent pour cell_id={cid}")
        cells_out.append(
            {
                ...
                "bourg": {
                    "habitants_du_bourg": repartition_bourg.habitants_du_bourg,
                    "habitants_des_champs": repartition_bourg.habitants_des_champs,
                },
