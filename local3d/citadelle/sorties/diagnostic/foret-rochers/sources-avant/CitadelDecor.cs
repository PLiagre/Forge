...
namespace ForgeLocal3D
{
    ...
    public static class CitadelDecor
    {
        ...
        public static int Rocks=-1,DeadTrees=-1,Props=-1,Buildings=-1;
        ...
        {
            Rocks=DeadTrees=Props=Buildings=-1;placedProps=0;kinds.Clear();converted.Clear();
        ...
        static int Hamlet(Transform parent)
        {
            ...
            var rule=new Rule{maxRange=.9f,slope=.72f,margin=2,ground=Buildable,ignored=Landscape};
            int count=0;
            foreach(var (path,name) in new[]{("Buildings/ForestVillage/Smithy","forge"),("Buildings/Farm/Stables/Stable_2horse","ecurie")})
            {
                ...
                var go=Put(k,parent,name,bestAt,bestYaw,1,rule);count++;
        ...
        static int Boulders(VillageV2Builder.SceneSpec spec,Transform parent)
        {
            ...
            // Éboulis au pied des falaises, puis blocs isolés et petits groupes sur les pentes.
            foreach(var cliff in spec.instances.Where(i=>i.asset.StartsWith("falaise_")))
                for(int i=0,placed=0;i<8&&placed<3;i++)if(Rock(P(cliff.position)+Around(4,18),R(.7f,2.6f)))placed++;
            for(int i=0;i<160;i++)
            {
                var center=new Vector3(R(-290,290),0,R(-260,175));
                if(!Rock(center,R(1.2f,3.4f)))continue;
                for(int j=rng.Next(0,4);j>0;j--)Rock(center+Around(1.2f,4.5f),R(.35f,1.2f));
            }
            return n;
        }
