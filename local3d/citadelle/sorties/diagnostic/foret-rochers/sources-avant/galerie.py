...
for d in recipe['dispositions']:
    ...
    for i in r['instances']:
        if i['asset'].startswith('maison_'):
            x,y=xy(i['position']);kind=int(i['asset'][-1]);w=(6.2+kind*.36)*i['scale'][0]*3;depth=(7+kind%3)*i['scale'][1]*3
...
for d in recipe['dispositions']:
    ...
    terrain_cards.append(f'<article><img loading="lazy" src="villages/{d["id"]}/renders/unity_vallee.png" alt="Terrain adapté — {html.escape(d["label"])}"><div><h3>{html.escape(d["label"])}</h3><p>{sample["vegetation"]} touffes et buissons · {sample["rochers"]} affleurements avec collision</p><a href="villages/{d["id"]}/renders/unity_lande.png">Voir le sol et la végétation</a> · <a href="villages/{d["id"]}/renders/unity_roche.png">Voir la roche</a></div></article>')
if terrain_cards:
    section='<h2>Le terrain, la roche et la lande</h2><p>Terrain Sample Project de Unity, adapté au rendu URP et à l’hiver de la citadelle. Roche, terre et gravier se mêlent à la neige. Les chemins et les clairières restent libres.</p><div class="grid">'+''.join(terrain_cards)+'</div>'
...
scene_date=max((OUT/'villages'/d['id']/(d['id']+'.blend')).stat().st_mtime for d in recipe['dispositions'])
