    ...
    for name,builder,kind in assets.jobs(TEX):
        ...
        for o in lods:
            ...
            if not source.exists() and o.name.endswith('_LOD0') and kind in ('batiment','repere','module'):
                ...
                bevel.width=.028;bevel.segments=1;bevel.limit_method='ANGLE'
...
def scene(name,seed,quick=False):
    ...
    for i,(x,y) in enumerate([(-20,5),(23,7),(-22,28),(20,30),(-38,20),(38,22)]):
        if alt:x+=2;y-=2
        place('maison_gothique_'+str(1+i%5),x,y,42.15,90 if x<0 else -90,(.78,.82,1.05))
    for i in list(instances):
        if not i['asset'].startswith('maison_') or i['position'][2]<40:continue
        k=int(i['asset'][-1]);x,y,z=i['position'];s=i['scale'];a=math.radians(i['rotation'])
        # Une lucarne dans le pan droit, au tiers de la hauteur de toiture.
        dx=(6.2+k*.36)*.30*s[0];dy=.5*s[1]
        place('lucarne_gothique',x+dx*math.cos(a)-dy*math.sin(a),y+dx*math.sin(a)+dy*math.cos(a),z+(5.6+k%3*1.2+(5.5+k*.18)*.37)*s[2],i['rotation']+90,tuple(s))
    ...
    sc=bpy.context.scene;sc.render.engine='CYCLES';sc.cycles.samples=32 if quick else 96;sc.cycles.use_denoising=True
