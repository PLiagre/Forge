...
def roof(m,w,d,z,rise,seed=0):
    ...
    for side in (-1,1):
        ...
        m.detail=2
        # Une couverture continue, bordée d'ondulations et plus mince sous le vent.
        segments=max(12,int(d*5));bands=8
        for i in range(segments):
            y1=-d/2+i*d/segments;y2=-d/2+(i+1)*d/segments
            def limit(yy):
                return min(.99,.68+.12*math.sin(yy*1.1+seed)+.08*math.sin(yy*2.7+seed*.3)) if side==1 else min(1,.90+.08*math.sin(yy*1.7+seed))
            for j in range(bands):
                fs=[j/bands*limit(y1),(j+1)/bands*limit(y1),(j+1)/bands*limit(y2),j/bands*limit(y2)]
                points=[]
                for f,yy in zip(fs,(y1,y1,y2,y2)):
                    thick=.085+.08*math.sin(math.pi*f)
                    points.append((side*w*.5*f,yy,z+rise*(1-f)+thick))
                m.surface(points,[order],mat('neige_givre'))
            f1=limit(y1);f2=limit(y2)
            m.surface([(side*w*.5*f1,y1,z+rise*(1-f1)+.09),(side*w*.5*f2,y2,z+rise*(1-f2)+.09),
                       (side*w*.5*f2,y2,z+rise*(1-f2)+.045),(side*w*.5*f1,y1,z+rise*(1-f1)+.045)],[(0,1,2,3)],mat('neige_ombre'))
        m.detail=1
...
def spire(m,x,y,z,r,h):
    ...
    for i in (1,2,5):
        ...
        m.surface([(x+r*math.cos(a),y+r*math.sin(a),z+.045),(x+r*math.cos(b),y+r*math.sin(b),z+.045),(x,y,z+h+.04)],[(0,1,2)],mat('neige_givre'))
    for i in range(8):
        a=i*math.tau/8
...
def buttress():
    ...
    m.beam((0,0,19.35),(5.5,0,24.35),.19,mat('neige_givre'))
    m.detail=0
...
def pine(index):
    ...
    for j in range(10):
        ...
        for k in range(6):
            ...
            m.surface(vs,[(0,1,4),(1,2,4),(2,3,4),(3,0,4),(0,5,1),(1,5,2),(2,5,3),(3,5,0)],mat('aiguilles_sombres'))
            if rng.random()>.18:
                ...
                m.surface(pts,[(0,1,4),(1,2,4),(2,3,4),(3,0,4)],mat('neige_givre'))
            m.detail=0
            if j<6:
                for side in (-1,1):
                    start=forward*rr*.55+Vector((0,0,zz-.13))
                    end=forward*rr*.88+across*rr*.33*side+Vector((0,0,zz-.68))
                    m.surface([tuple(start),tuple(end),tuple(tip)],[(0,1,2)],mat('aiguilles_sombres'))
            m.detail=2
    return m

def crag(index):
    rng=random.Random(index+918);m=Mesh();n=18;rings=16;vs=[]
    for j in range(rings):
        z=j/(rings-1)*20
        for i in range(n):
            a=i*math.tau/n;r=(1.1-abs(j/(rings-1)-.38)*.70)*(1+.20*math.sin(i*2.7+index)+rng.uniform(-.09,.09))
            r+=.12*math.sin(j*1.7+i*.45+index)
            vs.append((math.cos(a)*5*r+math.sin(z*.12+index)*1.2,math.sin(a)*4*r,z+rng.uniform(-.32,.32)))
    for j in range(rings-1):
        for i in range(n):
            a=j*n+i;b=j*n+(i+1)%n;c=(j+1)*n+(i+1)%n;d=(j+1)*n+i
            m.surface([vs[a],vs[b],vs[c],vs[d]],[(0,1,2),(0,2,3)],mat('roche_noire' if (i+index)%5 else 'roche_claire'))
...
def mountain_field(x,y,index):
    ...
    for frequency,amount in ((5,15),(11,7),(24,3),(51,1.2)):
...
def mountain(index):
    ...
    cold=np.array((.78,.84,.88));stone=np.array((.19,.215,.23));rgb=stone[None,None,:]*(1-cover[...,None])+cold[None,None,:]*cover[...,None]
    rgb*=np.clip(.90+grain[...,None]*.25,.7,1.05)
