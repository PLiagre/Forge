...
Faire porter au bandeau du tableau de bord (`vues/tableau/`) le partage bourg /
...
`vues/tableau/snapshot_loader.py` agrège déjà `population` en `agregats_monde`. Si
...
Ce qui rend ce lot caduc : si `vues/tableau/snapshot_loader.py::agregats_monde`
...
```bash
grep -n "habitants_du_bourg\|habitants_des_champs" vues/tableau/snapshot_loader.py
grep -n "bourg" sim/snapshot_export.py
grep -n "kpi-bourg\|kpi-champs" vues/tableau/static/index.html
```
...
même titre que `vues/tableau/` agrège déjà `population` sans la recalculer.
...
`vues/tableau/README.md` redit pour ce paquet précis : *« Aucune logique métier :
le paquet lit un snapshot et montre les cellules déjà photographiées. »*
`vues/tableau/` est un second regard sur une vue déjà figée par 051 ; il ne la
...
`vues/tableau/snapshot_loader.py::agregats_monde` gagne deux entrées, au même
...
Cette lecture réutilise `vues.tableau.classify.classify` ; elle ne convertit
...
`vues/tableau/static/index.html` gagne deux cartes dans `#kpis`, sur le modèle des
sept déjà présentes : `id="kpi-bourg"` (libellé « Habitants du bourg ») et
`id="kpi-champs"` (libellé « Habitants des champs »). `vues/tableau/static/app.js`
...
`#kpis` est déjà une grille `auto-fit` (`vues/tableau/static/style.css`, règle
...
`vues/tableau/snapshot_loader.py::proposed_layers`, `_valeur_couche` et
`agregats_couche` ne sont pas touchés : la couche par cellule n'est pas ce
lot. `vues/tableau/server.py`, `vues/tableau/classify.py`, `vues/tableau/svg_proof.py` ne
...
En écriture : `vues/tableau/snapshot_loader.py` (les deux entrées de
`agregats_monde`), `vues/tableau/static/index.html` (les deux cartes de `#kpis`),
`vues/tableau/static/app.js` (les deux appels dans `showKpis()`), et
`vues/tableau/tests/test_viewer_v0b.py` pour y **ajouter** des cas — c'est le
...
`sim/constants.py`), `vues/tableau/server.py`, `vues/tableau/classify.py`,
`vues/tableau/svg_proof.py`, `vues/tableau/static/style.css`, `vues/tableau/README.md`,
`vues/tableau/__main__.py`, la carte figée, `data/`, `ROADMAP.md` (au-delà de la
...
Commande : `python3 -m pytest vues/tableau/tests/test_viewer_v0b.py -k bourg -q`.
...
dans `vues/tableau/static/index.html`. `test_dashboard_html_porte_les_kpis`
reste inchangé. Un contrôle de code source vérifie en outre que
`vues/tableau/static/app.js` référence `monde.habitants_du_bourg` et
...
Commandes : `python3 -m pytest vues/tableau/tests/test_viewer_v0b.py -k bourg -q`,
puis `python3 -m vues.tableau --snapshot /tmp/monde.json` avec un snapshot réel
...
Un contrôle parcourt le code source de `vues/tableau/snapshot_loader.py` et de
`vues/tableau/static/app.js`, et échoue si l'un des deux référence
...
`bourg_depuis_monde` dans une copie de `vues/tableau/snapshot_loader.py` :
...
ne passe jamais par `vues/tableau/`.
...
- tous les contrôles déjà présents dans `vues/tableau/tests/test_viewer_v0b.py`
...
- un seuil, un drapeau, une classification « ceci est une ville » dans le
  tableau de bord ;
- la preuve SVG (`vues/tableau/svg_proof.py`), le serveur (`vues/tableau/server.py`),
  la classification des états (`vues/tableau/classify.py`) : aucun ne change de
  comportement, le bourg suit le même contrat `absent / non_calculé / zéro /
  valeur` que tout autre champ du bandeau ;
- `vues/tableau/README.md` : sa description du tableau de bord (« bandeau de
