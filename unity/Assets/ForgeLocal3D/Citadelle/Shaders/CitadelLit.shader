Shader "Forge/CitadelLit"
{
    Properties
    {
        ...
        _UseAtlas("Zone de texture Blacksmith",Float)=0
        _AtlasRect("Rectangle dans la texture",Vector)=(0,0,1,1)
        ...
        _SpeedTreeMask("Masque SpeedTree : lissage, métal, occlusion",Float)=0
        _Foliage("Feuillage",Float)=0
    ...
    SubShader
    {
        ...
        float4 _BaseMap_ST,_BaseColor,_EmissionColor,_AtlasRect;
        float _UseAtlas;
        float _Cutoff,_AlphaClip,_Wind,_SnowFactor,_Smoothness,_Banner,_HasGlossMap,_Metallic,_BumpScale,_Weathering;
        float _WindHeight,_HasMaskMap,_SnowBase,_SpeedTreeMask,_Foliage;
        CBUFFER_END
        ...
        half4 Frag(V i,bool front:SV_IsFrontFace):SV_Target
        {
            ...
            float2 sampleUV=lerp(i.uv,_AtlasRect.xy+frac(i.uv)*_AtlasRect.zw,_UseAtlas);
            float2 derivativeScale=lerp(float2(1,1),_AtlasRect.zw,_UseAtlas);
            float2 gx=ddx(i.uv)*derivativeScale,gy=ddy(i.uv)*derivativeScale;
            half4 tex=SAMPLE_TEXTURE2D_GRAD(_BaseMap,sampler_BaseMap,sampleUV,gx,gy)*_BaseColor;
            ...
            float3 map=UnpackNormal(SAMPLE_TEXTURE2D_GRAD(_BumpMap,sampler_BumpMap,sampleUV,gx,gy));
            map.xy*=_BumpScale;
            ...
            // Les cartes de branches sont à deux faces : la neige couvre leurs aiguilles
            // même quand la normale simplifiée de la carte est presque verticale.
            float branchCover=saturate(abs(normalize(i.normal).y)*.50+.42+.10*sin(i.world.y*2+i.world.x*.9));
            snow=max(snow,saturate(_Foliage*_SnowBase*_SnowFactor*branchCover));
            ...
            if(_HasMaskMap>.5)
            {
                ...
                if(_SpeedTreeMask>.5)
                {surface.metallic=mask.g*_Metallic;surface.smoothness=mask.r*_Smoothness;surface.occlusion=lerp(1,mask.b,.7);}
            }
            ...
            surface.emission+=tex.rgb*_AlpineAmbient.rgb*_Foliage*.18;
            ...
            return half4(MixFog(lit,i.fog),_AlphaClip>.5?saturate((tex.a-_Cutoff)/max(fwidth(tex.a),.001)+.5):1);
        }
